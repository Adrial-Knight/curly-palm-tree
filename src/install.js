...
const tables = ["USERS", "ARTICLES", "COMMENTS", "FAVORITES", "VOTES", "INTERACTIONS"]
...
async function createTables(db){
  ...
  for (var i = 0; i < u_pseudos.length; i++) {
    await db.run(sql_insert, [u_pseudos[i], u_mails[i], u_passwords[i], date])
  }
  ...
  await db.run(`INSERT INTO INTERACTIONS (i_article, i_user, i_ref, i_date) VALUES (?, ?, ?, ?)`, ['1', 2, 1, date])
}
